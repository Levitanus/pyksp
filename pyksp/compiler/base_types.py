...
class AstAddString(AstBase):
    ...
    def __iadd__(self, other):
        raise NotImplementedError('method __iadd__ is not implemented')
...
class AstOperator(AstBase):
    ...
    def unpack_args(self, *args):
        ...
        for idx, arg in enumerate(args):
            ...
            if not isinstance(arg, (int, str, float)):
                raise TypeError(f'arg {arg} has to be resolved to str,' +
                                ' int or float')
            new.append(arg)
        if len(new) == 1:
            new = new[0]
    ...
    def standart(self, string: str, val1: Union[int, str, float, KspObject],
    ...
    def __isub__(self, other):
        raise NotImplementedError('method __isub__ is not implemented')
    ...
    def __imul__(self, other):
        raise NotImplementedError('method __imul__ is not implemented')
    ...
    def __itruediv__(self, other):
        raise NotImplementedError('method __itruediv__ is not implemented')
    ...
    def __ifloordiv__(self, other):
        raise NotImplementedError('method __ifloordiv__ is not implemented')
    ...
    def __imod__(self, other):
        raise NotImplementedError('method __imod__ is not implemented')
    ...
    def __ipow__(self, other):
        raise NotImplementedError('method __ipow__ is not implemented')
...
class KspVar(KspObject):
    ...
    def read(self):
        ...
        if self._read:
            warn('read has been called yet', category=Warning, stacklevel=2)
...
class KspNumeric(KspVar):
    ...
    class TypeWarn(Warning):
        ...
        def __init__(self, val):
            super().__init__(f'Value {val} (type{type(val)}) ' +
...
class KspArray(KspVar):
    ...
    def append(self, val):
        ...
        if not isinstance(val, self.ref_type):
            raise TypeError(f'has to be instance of {self.ref_type}')
        if not self.in_init():
    ...
    def _check_cashed_item(self, idx):
        ...
        if val is None:
            ...
            val = self._seq[idx]
        if not isinstance(val, self.ref_type):
            raise TypeError(f'items has to be of type {self.ref_type}; ' +
                            f'returned {val} ' + f'({type(val)})')
        if isinstance(val, KspVar):
            val = val._get_runtime()
        if isinstance(val, AstBase):
            val = val.get_value()
        item = self._item_type(
            name=f'{self.name()}[{idx}]', value=val, is_local=True)
    ...
    def __iter__(self):
        raise NotImplementedError('can not be used in iter.' +
