...
class FrameVarTest(FrameVar):
    ...
    def __call__(self):
        if self.len == 1:
            raise TypeError("it shouldn't be created not within array")
        return self.val


class FrameVarCompile(FrameVar):
    ...
    def __call__(self):
        if self.len > 1:
            raise TypeError("it shouldn't be called within array")
        return self.val
...
class Stack(KSP):
    ...
    def push_arg(self, name, val, count):
        ...
        if not is_arr:
            ...
            var = FrameVarCompile(name, val, length=1)
            return var, count + 1

        if not KSP.is_under_test():
            return self._arr_compile(val, idx, count, name)
        return self.__arr_test(val, name, count)

    def __arr_test(self, val, name, count):
        var = FrameVarTest(val, name)
        count += len(val)
        return var, count

    def _arr_compile(self, val, idx, count, name):
        ...
        var = FrameVarCompile(name, val, length)
