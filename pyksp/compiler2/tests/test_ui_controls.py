...
from pyksp.compiler2.abstract import Output
from pyksp.compiler2.abstract import KSP
from pyksp.compiler2.abstract import KspObject
from pyksp.compiler2.base_types import KspVar

from pyksp.compiler2.dev_tools import unpack_lines
from pyksp.compiler2.bi_ui_controls import *
from pyksp.compiler2.bi_engine_par import NI_BUS_OFFSET

from pyksp.compiler2.native_types import kInt
from pyksp.compiler2.native_types import kArrInt

from pyksp.compiler2.conditions_loops import For, check
...
class TestKspNativeControl(DevTest):
    ...
    def test_child(self):
        ...
        self.assertEqual(child.hide.val, HIDE_PART_BG.id)
        ...
        self.assertEqual(knob.unit.val, KNOB_UNIT_DB.id)
        ...
        self.assertEqual(button.text.val, '5')
    ...
    def test_generation(self):
        ...
        print(isinstance(button1.var, KspVar))
        print(isinstance(button1.var, KspObject))
        ...
        switch.y <<= 9
        button2.x <<= 10
        print('KspObject instances():', KspObject.instances())
        print('KspObject _instances:', KspObject._instances)
        Output().release()
        init = list()
        init.extend(KspObject.generate_all_inits())
        # init.extend(KspNativeControlMeta.generate_init_code())
        init.extend(current)
        print(unpack_lines(init))
