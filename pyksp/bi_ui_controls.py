...
class kWidget(metaclass=WidgetMeta):
    ...
    def __init__(self, parent: object=None, x: int=None, y: int=None,
        ...
        self._parent = parent
        self._childs: List[kWidget] = list()
        self._grid: Optional[WidgetGrid] = None
...
class ControlParFunc:
    def _resolve_parameter(self, ctrl, parameter):
        ...
        if isinstance(parameter, str):
            if parameter in bControlParVar.shorts:
                parameter = bControlParVar.shorts[parameter]
            else:
...
class ControlPar():
    ...
    def refresh(self):
        self._controls = dict()
        self._obj_count = int()
        self._values = list([list()] * self._init_size)
        self._vars = list()
    ...
    def _init_control(self, control_obj: 'KspNativeControl',
        ...
        self._controls_by_idx.append(control_obj)
        self._obj_count += 1
    ...
    def generate_init_code(self):
        if not self._used:
            return [None] * 3
        if self._size > 1:
            raise NotImplementedError('can not generate for arrays')
        values_seq = list()
        active_seq = list()
        out = list()
        for idx, item in enumerate(self._values[0]):
            # item = item[0]
            if item._init_value:
                values_seq.append(item._init_value)
                active_seq.append(1)
            else:
        ...
        return out, values_arr, active_arr
...
class KspNativeControlMeta(WidgetMeta):
    ...
    def __new__(self, name, bases, dct):
        ...
        KspNativeControlMeta.classes.append(cls)
    ...
    def generate_init_code():
        ...
        for obj in KspNativeControlMeta.objects:
            if obj.x._init_value:
                params['x'].append(obj.x._init_value)
            else:
                params['x'].append(-1)
            if obj.y._init_value:
                params['y'].append(obj.y._init_value)
            else:
                params['y'].append(-1)
            if obj.width._init_value:
                params['width'].append(obj.width._init_value)
            else:
                params['width'].append(-1)
            if obj.height._init_value:
                params['height'].append(obj.height._init_value)
            else:
        ...
        Output().set(out)
        ids_arr = ControlId._controls_arr
        out.extend(For.init_arrays())
        rnd_idx = kInt(name='rnd_idx')
        with For(len(ids_arr)) as indicies:
            for idx in indicies:
                with If(all_x[idx] != -1):
                    set_control_par(ids_arr[idx], 'x', all_x[idx])
                with If(all_y[idx] != -1):
                    set_control_par(ids_arr[idx], 'y', all_y[idx])
                with If(all_width[idx] != -1):
                    set_control_par(ids_arr[idx], 'width', all_width[idx])
                with If(all_height[idx] != -1):
                    set_control_par(ids_arr[idx], 'height', all_height[idx])
        Output().release()
